...
- `codex`: reads the user-visible `Name` only from the index shared with the Desktop left sidebar and uses local task state for ID/default-cwd/archive/recency/owner metadata. The full task ID selects an existing task, while the current configuration supplies each delivery workspace. It asks the Desktop owner to start or steer the real turn through Desktop IPC. app-server `thread.name` and SQLite `threads.title` never override the sidebar Name or participate in same-name lookup.
...
## Saving configuration and recovering conflicts

The Save command on the message adapter page submits only the current route and preserves drafts on other routes. Unsaved changes are computed against the last acknowledged configuration; reverting an edit clears the indicator. Edits made during a save survive delayed responses.

After a timeout or disconnect, the page shows a pending confirmation state and queries the original operation's durable receipt before another write. Browser storage contains only operation identifiers and digests, never a copy of configuration credentials. Pending operations left by older pages use the same recovery endpoint. If confirmation remains unavailable, edits are preserved and Save retries confirmation.

Independent field edits are merged. Overlapping changes preserve the draft and identify the conflicting field. Reload current route discards that route's draft after confirmation while preserving other drafts. Persistence and runtime activation are reported separately. Routes sharing a persona still share one authoritative persona configuration.

The overview can edit multiple routes. Save submits each changed route in its own transaction; a conflict preserves remaining drafts.
