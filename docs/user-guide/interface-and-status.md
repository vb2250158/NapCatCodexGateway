...
Agent 按当前人格 `planWorkflow.roles` 选择状态 key。默认模板中，仍在分析时显示“分析中”；只有分析已经完成，但现有信息仍无法形成可审批的具体方案，且缺失信息会影响原因、改法、范围或验收合同时，才显示“待补充信息”。暂未复现、疑似历史已修复、等待目标包、等待 QA 或等待是否关闭都不属于该状态。完整 `approvalRequest` 等待回执显示“待审批”；批准或用户明确直接授权后的实施与开发验证显示“执行中”；开发侧完成但缺目标包或纳入证明显示“等待打包”；目标包确认但缺 QA 结论显示“等待 QA”；确认问题无效或历史已修复且无需验收时凭证据显示“关闭”。人格可以调整 key 与显示名称，界面始终显示配置 label。归档是独立变量，不会把终态改成另一个状态。
