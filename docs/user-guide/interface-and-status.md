...
浏览器地址是当前人格与页面的依据。刷新、从收藏打开、关闭浏览器后重新访问，都会恢复 URL 指定的 Route 和页面，并同步顶栏下拉及左侧选中状态。切换人格保留当前页面和查询参数，浏览器前进、后退也会同步选择。加载插件时保留原目标地址；URL 指定的 Route 不存在时显示错误，不自动切到第一项。

Route 配置名会进行 URL 编码。打开任一 Route 作用域链接后，页面会先选中该 Route；切换顶栏 Route 时，当前浏览器会话的地址会同步为新 Route 的同类页面路径。需要收藏、重新打开或发送给同一局域网中的已授权设备时，应使用包含访问密钥的完整链接，不要直接复制已经自动移除密钥的地址栏。
...
若 Rabi PC 已开启全局 RabiLink Relay 连接，并在 Relay 管理后台属于当前账号，可先登录 `https://relay.example.com/manage`，再打开：

```text
https://relay.example.com/manage/<账号>/<RabiGUID>/#/routes/<Route配置名>/knowledge
```
...
“计划与记忆”的内容区直接从统计卡片开始，下方保留分类、搜索、计划目录和列表；页面标题与人格切换仍在顶栏。 目录标题、数量与排序按钮保持一行。桌面布局两栏之间没有留白，拖动分隔线可调整目录宽度；缩到 160px 以下折叠为一条线，向右拖动该线即可恢复。分隔线也支持方向键调整和 Enter 折叠/展开；窄屏仍按上下布局显示。
...
计划卡片的分类、状态名称、状态色板、排序和审批说明都由 Manager 返回。RibiWebGUI 顶部显示“当前计划 / 近期记忆 / 沉淀记忆 / 已归档”四个标签：“当前计划”展示 `archiveStatus=未归档` 且状态配置的 `views` 包含 current 的计划；其它未归档分类也按配置视图展示。“已归档”展示 `archiveStatus=已归档` 的计划和已经作为沉淀输入的来源记忆。已归档计划不参加关键词搜索或消息召回，只能通过明确计划 ID 或“已归档”视图读取。计划目录只列当前页签与筛选结果，卡片仍展示完整步骤、附件、审批合同和工作留痕。
...
“计划与记忆”页不直接读取 `data/`，也不在浏览器里重新解释状态。`plan.status` 只保存当前人格配置中的状态 key；Manager 同时返回该 key 对应的名称、说明、颜色、排序和视图，WebGUI 与托盘只显示这些配置结果。Agent 可通过 Manager 的人格计划状态接口新增、调整或移除状态，不需要修改程序。步骤、等待原因和审批资料不能产生第二个状态。

Agent 按当前人格 `planWorkflow.roles` 选择状态 key。默认模板中，仍在分析时显示“分析中”；只有分析已经完成，但现有信息仍无法形成可审批的具体方案，且缺失信息会影响原因、改法、范围或验收合同时，才显示“待补充信息”。暂未复现、疑似历史已修复、等待目标包、等待 QA 验收或等待是否关闭都不属于该状态。完整 `approvalRequest` 等待回执显示“待审批”；批准或用户明确直接授权后的实施与开发验证显示“执行中”；开发侧完成但缺目标包或纳入证明显示“等待打包”；目标包确认但缺 QA 结论使用状态 key“等待 QA”，显示“等待 QA 验收”；确认问题无效或历史已修复且无需验收时凭证据显示“关闭”。人格可以调整 key 与显示名称，界面始终显示配置 label。归档是独立变量，不会把终态改成另一个状态。
...
长计划列表保持正常页面滚动，只有外侧计划目录限制在视口高度内独立滚动。滚动右侧计划时，页面使用浏览器可见性观察更新目录中的当前阅读项；只有高亮项离开目录自身的可见区域时才调整目录滚动，不对整页滚动执行持续的全列表扫描。点击目录跳转时，页面先等目标卡片挂载，再暂时锁定所选高亮并平滑滚动；滚动结束后恢复当前阅读项观察，避免沿途计划让目录光标连续跳动。计划分类、搜索和刷新工具栏在桌面宽度下会吸附在固定顶栏下方；目录跳转为卡片标题预留工具栏高度，不会把定位目标遮住。窄屏下工具栏恢复普通页面流，避免双行控件长期占用可视区域。展开详情不执行高度动画，审批输入也不自动反复增高。
...
顶栏路由下拉决定当前查看和修改哪一条 Route。只有消息适配器、人格配置（含文档和同步子页）、计划与记忆提供此入口；其他页面隐藏下拉，继续沿用当前选择。切换前如果存在未保存修改，界面会请求确认。

下拉选中项和候选项显示人格名称；桌面宽度下，当前 Route 配置名显示在顶栏标题下方。
...
自动化页面的“可用模板变量”按钮会打开变量说明弹窗，查看后可关闭；变量列表不再常驻页面。

在“Agent Hook → 事件投递”添加规则，按“事件 → 条件 → 投递至消息端”配置。当前支持“任务完成”事件，只有通过完成检查的轮次才会发送。条件可添加或删除，目前包括“限定项目”、“必须绑定计划”、“限定聊天会话”和“排除聊天会话”；多个条件必须全部满足，没有条件时匹配全部 Codex 任务。

聊天会话条件支持按名称搜索、多选和加载更多；同一个限定列表内满足任意一个即可，排除列表优先，其他条件仍需全部满足。会话按完整 ID 匹配，改名不影响规则；名称仅用于显示。空列表不能启用，读取列表只由按钮触发。

消息端选择 NapCat 时，填写消息路线、QQ 账号、类型（群／个人 QQ）和群号或 QQ 号；选择语音时，使用所选路线的语音合成与播放设置，不显示 QQ 参数。旧的群投递规则会迁移到此结构，保持原项目、计划条件、目标群和启用状态；保存后只保留新结构。新规则默认关闭，配置完整后启用并保存人格。

限定项目时，同一仓库内不同目录的 Codex 项目分别匹配，工作树沿用项目在仓库中的相对目录，非 Git 项目按完整目录匹配。通知显示任务名、已绑定的计划名和最终回复，不展示项目路径或会话 ID。任务名优先读取 Codex 当前标题，读取不到时使用计划绑定中保存的任务名；两者都缺失时显示“未命名任务”。“必须绑定计划”条件只接受当前人格计划的任务绑定。

同一任务同一轮次发往同一目标只投递一次；重复事件复用发送回执。发送状态不明确时不会自动重发，避免重复通知，可在消息路线的发送日志中检查。缺少完成轮次或最终回复时不发送。规则默认关闭，不会因添加规则立即发送历史消息。

“人格 → 自动化”分为“接收消息”“定制任务”和“Agent Hook”。Hook 开关随人格保存在 `personaConfig.json`；更换 Agent 端后保留设置。同一人格的消息路线共用这些开关。旧 Route 配置在保存或显式迁移时转入人格文件。

Agent 端只提供“更新 Hook 到 Agent”按钮，用于首次安装或后续更新事件埋点。Codex 使用官方插件安装入口；DSH 安装到本机 `web` profile，重新加载插件或重启 DSH 后生效。两套安装包位于 `plugins/rabi-codex-context/` 和 `plugins/rabi-dsh-context/`。

“提交”和“提交并投递”在输入区右下方紧邻排列，间距 8px；窄屏时两按钮在说明文字下方并排显示。“提交”只保存输入，“提交并投递”保存后通知 Agent。

### 审批阅读与附件预览

审批区优先显示待决定事项、推荐方案和附件预览。图片与视频可放大查看，Markdown 报告使用正文摘要卡片并支持全文预览；其他文件保留打开入口，与描述区使用相同预览方式。

方案依据与备选、文件改动、执行命令、验收与回退、回执及历史意见默认折叠，点击标题展开。空的文件、命令、变更区块不显示；实际配置与外部影响、缺失审批资料警告保持可见，审批条件不变。

收起左侧导航后，计划页扩展到可用宽度（最大 1880px），两侧仍有留白；移动端保留 16px 边距。
