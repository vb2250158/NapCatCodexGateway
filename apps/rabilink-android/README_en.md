...
[0.3.22 recording interface](../../docs/rabilink-mobile-recording-ui_en.md): three capture modes, mutually exclusive audio/video, and separate Records, Messages and Devices navigation.
...
- Home now provides Pause, Audio recording and Video recording; the conversation list is under Messages. Every configured persona remains visible even when its Route or chat capability is disabled; such rows retain configuration guidance instead of disappearing. Tapping a persona with an enabled RabiLink message adapter opens chat; Back returns to choose another persona. Settings, health, and glasses remain separate surfaces.
